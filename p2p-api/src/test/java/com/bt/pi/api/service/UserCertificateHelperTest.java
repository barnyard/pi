...
public class UserCertificateHelperTest {
    ...
    @Before
    public void before() throws CertificateEncodingException {
        bytesString = "SoyBean Field";

        blockingDhtCache = mock(BlockingDhtCache.class);
        userId = mock(PId.class);
        certificate = mock(Certificate.class);
        when(certificate.getEncoded()).thenReturn(bytesString.getBytes());

        userCertificateHelper = new UserCertificateHelper();
        userCertificateHelper.setUserCache(blockingDhtCache);
        user = mock(User.class);
    }

    @SuppressWarnings("unchecked")
    @Test
    public void createUserCertificateShouldReturnTrue() throws Exception {

        // setup
        doAnswer(new Answer<Object>() {
            public Object answer(InvocationOnMock invocation) throws Throwable {
                UpdateResolver resolver = (UpdateResolver) invocation.getArguments()[1];
                resolver.update(null, null);
                return null;
            }
        }).when(blockingDhtCache).update(eq(userId), isA(UpdateResolver.class));

        // act
        boolean isNewUser = userCertificateHelper.addCertificateToUser(userId, null, certAsByteArray);

        // assert
        assertTrue(isNewUser);
        verify(user).setCertificate(eq(certificate.getEncoded()));
    }

    @SuppressWarnings("unchecked")
    @Test
    public void createUserCertificateShouldReturnFalseIfUserExists() throws GeneralSecurityException {
        ...
        verify(user).setCertificate(certificate.getEncoded());

    }

    @SuppressWarnings("unchecked")
    @Test
    public void createUserCertificateShouldUpdateCertificateAndUserEntityInDht() throws Exception {
        // setup
        final User userFromDht = mock(User.class);
        doAnswer(new Answer<Object>() {
            @Override
            public Object answer(InvocationOnMock invocation) throws Throwable {
                ((UpdateResolver<User>) invocation.getArguments()[1]).update(userFromDht, null);
                return null;
            }
        }).when(blockingDhtCache).update(eq(userId), isA(UpdateResolver.class));
        when(user.getCertificate()).thenReturn(bytesString.getBytes());

        // act
        userCertificateHelper.updateUserCertificate(userId, user);

        // assert
        verify(userFromDht).setCertificate(eq(user.getCertificate()));
    }
}
