 ...
 * 
 * For input headers it is possible to use the MessageByte for Fileds - so no GC will be generated.
 * 
...
public class MimeHeaders {
    ...
    public void removeHeader(String name, String str) {
        for (int i = 0; i < count; i++) {
            if (headers[i].getName().equalsIgnoreCase(name) && getValue(i) != null && getValue(i).toString() != null && getValue(i).toString().matches(str)) {
                removeHeader(i--);
            }
        }
    }
...
/**
 * Enumerate the distinct header names. Each nextElement() is O(n) ( a comparation is done with all previous elements ).
 * 
