...
public class Utils {
    ...
    static String makeCanonicalString(String method, String bucketName, String key, Map<String, String> pathArgs, Map<String, List<String>> headers, String expires) {
        ...
        for (Entry<String, String> interestingHeader : interestingHeaders.entrySet()) {
            if (interestingHeader.getKey().startsWith(AMAZON_HEADER_PREFIX)) {
                buf.append(interestingHeader.getKey()).append(':').append(interestingHeader.getValue());
            } else {
                buf.append(interestingHeaders.get(interestingHeader.getKey()));
            }
            buf.append("\n");
        }
    ...
    public static String convertPathArgsHashToString(Map<String, String> pathArgs) {
        ...
        if (pathArgs != null) {
            for (Entry<String, String> pathArg : pathArgs.entrySet()) {
                if (firstRun) {
                    firstRun = false;
                ...
                pathArgsString.append(pathArg.getKey());
                if (pathArg.getValue() != null) {
                    pathArgsString.append("=");
                    pathArgsString.append(pathArg.getValue());
                }
            }
        }
    ...
    static boolean validateBucketName(String bucketName, CallingFormat callingFormat, boolean located) {
        if (callingFormat == CallingFormat.getPathCallingFormat()) {
            ...
            return null != bucketName && bucketName.length() >= MIN_BUCKET_LENGTH && bucketName.length() <= MAX_BUCKET_LENGTH && bucketName.matches(BUCKET_NAME_REGEX);
        } else {
    ...
    static boolean isValidSubdomainBucketName(String bucketName) {
        ...
        return null != bucketName && bucketName.length() >= MIN_BUCKET_LENGTH && bucketName.length() <= MAX_BUCKET_LENGTH && !bucketName.matches(IPv4_REGEX) && bucketName.matches(BUCKET_NAME_REGEX);
    }
