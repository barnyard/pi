...
/**
 * This class mimics the behavior of AWSAuthConnection, except instead of actually performing the operation,
 * QueryStringAuthGenerator will return URLs with query string parameters that can be used to do the same thing. These
 * parameters include an expiration date, so that if you hand them off to someone else, they will only work for a
...
public class QueryStringAuthGenerator {
    ...
    public QueryStringAuthGenerator(String awsAccessKeyId, String awsSecretAccessKey, boolean isSecure, String server, CallingFormat callingFormat) {
        this(awsAccessKeyId, awsSecretAccessKey, isSecure, server, isSecure ? Utils.SECURE_PORT : Utils.INSECURE_PORT, callingFormat);
    }
    ...
    public String listBucket(String bucket, String prefix, String marker, Integer maxKeys, Map<String, List<String>> headers) {
        return listBucket(bucket, prefix, marker, maxKeys, null, headers);
    }

    public String listBucket(String bucket, String prefix, String marker, Integer maxKeys, String delimiter, Map<String, List<String>> headers) {
    ...
    private Map<String, List<String>> mergeMeta(Map<String, List<String>> headers, Map<String, List<String>> metadata) {
        ...
        if (metadata != null) {
            for (Entry<String, List<String>> entry : metadata.entrySet()) {
                String metadataKey = Utils.METADATA_PREFIX + entry.getKey();
                if (merged.containsKey(metadataKey)) {
                    merged.get(metadataKey).addAll(entry.getValue());
                } else {
                    merged.put(metadataKey, entry.getValue());
                }
            }
        }
        return merged;
