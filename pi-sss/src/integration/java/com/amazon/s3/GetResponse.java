...
/**
 * A Response object returned from AWSAuthConnection.get(). Exposes the attribute object, which represents the retrieved
 * object.
 */
public class GetResponse extends Response {
    ...
    /**
     * Examines the response's header fields and returns a Map from String to List of Strings representing the object's
     * metadata.
     */
    private Map<String, List<String>> extractMetadata(HttpURLConnection connection) {
        ...
        for (Entry<String, List<String>> header : connection.getHeaderFields().entrySet()) {
            if (header.getKey() == null)
                continue;
            if (header.getKey().startsWith(Utils.METADATA_PREFIX)) {
                metadata.put(header.getKey().substring(Utils.METADATA_PREFIX.length()), header.getValue());
            }
        }
