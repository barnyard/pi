...
public abstract class RemoveResourceFromUserTaskProcessingQueueContinuationBase<T extends PiEntity> implements TaskProcessingQueueContinuation {
    ...
    public void receiveResult(final String uri, String nodeId) {
        ...
        dhtClientFactory.createReader().getAsync(resourcePastryId, new PiContinuation<T>() {
            ...
            public void handleResult(final T result) {
                if (result == null) {
                    LOG.error("Unable to find resource with uri: " + uri);
                    return;
                }
                ...
                dhtClientFactory.createWriter().update(userId, new UpdateResolvingPiContinuation<User>() {
                    ...
                    public void handleResult(User user) {
                        handleResultForUser(uri, removeResourceFromUserQueueId, resourcePastryId, result, ownerId);
                    }
                });
            }
        });
    ...
    protected void handleResultForUser(final String uri, final PId removeResourceFromUserQueueId, final PId resourcePastryId, final T result, final String ownerId) {
    ...
    protected void removeTaskProcessingQueue(final PId removeResourceFromUserQueueId, String uri) {
        LOG.debug(String.format("Resource %s status set to buried", uri));
        taskProcessingQueueHelper.removeUrlFromQueue(removeResourceFromUserQueueId, uri);
    }
